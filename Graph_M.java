...
public class Graph_M
{
    ...
    public int dijkstra(String src, String des, boolean nan) {
        ...
        for (String vtx : vtces.keySet()) {
            dist.put(vtx, Integer.MAX_VALUE);
        }
        dist.put(src, 0);
        ...
        while (!pq.isEmpty()) {
            ...
            if (map.containsKey(rem.vname)) {
                continue;
            }
            if(rem.vname.equals(des))
            {
                val = rem.cost;
                break;
            }
            map.put(rem.vname, rem.cost);

            for (String nbr : vtces.get(rem.vname).nbrs.keySet()) {
                if (map.containsKey(nbr)) {
                    continue;
                }
                Vertex k= vtces.get(rem.vname);
                int oCost = dist.get(nbr);
                int nCost;
                if(nan)
                    nCost = rem.cost + 120 + 40*k.nbrs.get(nbr);
                else
                    nCost = rem.cost + k.nbrs.get(nbr);


                if (nCost < oCost) {
                    dist.put(nbr, nCost);
                    pq.add(new Pair1(nbr, nCost));
                    path.put(nbr, rem.vname);
                }
            }
        }
        System.out.println("Shortest path: " + buildPath(src, des, path)); // Optional: Print the path
        return val;
    }
    ...
    public class Pair1 {
        ...
        public Pair1(String vname, int cost) {
            this.vname = vname;
            this.cost = cost;
        }
    }

    public class PairComparator implements Comparator<Pair1> {
        public int compare(Pair1 t, Pair1 o) {
            return t.cost - o.cost;
        }
    }

    private String buildPath(String src, String des, HashMap<String, String> path) {
        StringBuilder sb = new StringBuilder();
        while (!des.equals(src)) {
            sb.insert(0, " -> " + des);
            des = path.get(des);
        }
        sb.insert(0, src);
